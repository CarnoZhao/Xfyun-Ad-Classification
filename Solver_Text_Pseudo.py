...
class Model(pl.LightningModule):
    def __init__(self, **args):
        ...
        if "bert" in self.model_name or "rbt" in self.model_name:
    ...
    def prepare_data(self):
        ...
        if self.pseudo_from:
            ...
            label = np.load(self.pseudo_from).argmax(1)
            df = pd.DataFrame({"file_name": file_names, "label": label.tolist()})
            df = df.merge(pd.read_csv("./data/testB.tsv", sep = "\t"), on = "file_name")
            df = df.fillna("")
            self.df_train = pd.concat([self.df_train.iloc[:0], df]).reset_index(drop = True)
            self.df_valid = df.reset_index(drop = True)
        self.ds_train = self.Data(self.df_train, self.tokenizer, is_train = False, **self.args)
